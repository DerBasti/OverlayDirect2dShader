...
cbuffer Constants : register(b0) {
	...
	float4 targetColorLeft;
	float4 targetColorRight;
};
...
D2D_PS_ENTRY(main) : SV_TARGET{
	...
	float brightness = pow(nestedNoise(p), 1.5f);
	float4 fragmentColor = float4(brightness * backgroundColor.r, brightness * backgroundColor.g,
		brightness * backgroundColor.b, brightness * backgroundColor.a);
	float4 diffColorLeft = fragmentColor - targetColorLeft;
	float4 diffColorRight = fragmentColor - targetColorRight;
	if (x <= 0.25f) {
		float modifier = pow((0.25f - x) / 0.25f, 6.0f);
		fragmentColor -= (diffColorLeft * modifier);
	}
	else if (x >= 0.75f) {
		float modifier = pow((x - 0.75f) / 0.25f, 6.0f);
		fragmentColor -= (diffColorRight * modifier);
	}
